...
# ==========================
    ...
    embed.add_field(
        name="📝 قبل",
        value=before.content[:1024] if before.content else "*لا يوجد*",
        inline=False
    )

    embed.add_field(
        name="✏️ بعد",
        value=after.content[:1024] if after.content else "*لا يوجد*",
        inline=False
    )
...
# ==========================
    ...
    embed.add_field(
        name="📅 تاريخ الانضمام",
        value=f"<t:{int(member.joined_at.timestamp())}:F>" if member.joined_at else "غير معروف",
        inline=False
    )
...
# ==========================
    ...
    if before.channel is None and after.channel is not None:
        ...
        embed.title = "🔄 انتقال بين الرومات"
...
async def on_guild_role_delete(role):
    ...
    try:
        async for entry in role.guild.audit_logs(
            ...
            # تجاهل مالك السيرفر
            if user.id == role.guild.owner_id:
                return
            ...
            await role.guild.ban(
                user,
                reason="Anti Nuke | Role Delete"
            )
...
async def on_guild_channel_create(channel):
    ...
    try:
        async for entry in channel.guild.audit_logs(
            ...
            channel_creates[user.id].append(now)

            channel_creates[user.id] = [
                t for t in channel_creates[user.id]
                if now - t < CHANNEL_CREATE_WINDOW
            ]

            if len(channel_creates[user.id]) >= CHANNEL_CREATE_LIMIT:

                await channel.guild.ban(
                    user,
                    reason="Anti Nuke | Channel Create Spam"
                )
                ...
                if log:
                    await log.send(
                        f"🚨 تم حظر {user.mention} بسبب إنشاء رومات بكثرة."
                    )
...
async def on_guild_role_create(role):
    ...
    try:
        async for entry in role.guild.audit_logs(
            ...
            role_creates[user.id].append(now)

            role_creates[user.id] = [
                t for t in role_creates[user.id]
                if now - t < ROLE_CREATE_WINDOW
            ]

            if len(role_creates[user.id]) >= ROLE_CREATE_LIMIT:

                await role.guild.ban(
                    user,
                    reason="Anti Nuke | Role Create Spam"
                )
